...
class Chatbot:
    ...
    def run(self, serializable_chat, input):
    ...
    def deserialize_chat(chat_json):
        ...
        examples = [InputOutputTextPair(input_text=i, output_text=o) for i, o in chat_data["examples"]] if chat_data["examples"] else []
        message_history = []
        for msg in chat_data["message_history"]:
            role = msg['role']
            if role == 'bot':
                role = 'assistant'
            message_history.append(ChatMessage(content=msg['content'], author=role))
        return SerializableChat(
